...
#################################################################################
ptm <- proc.time()
pheno_result_v2 <- pheno_model(data$plotid, data$evi, data$doy, data$year)
proc.time() - ptm

res_nls_v2 <- data.frame(do.call(rbind, pheno_result_v2[[1]]))
res_spl_v2 <- data.frame(do.call(rbind, pheno_result_v2[[2]]))
results_v2 <- cbind(res_nls_v2, res_spl_v2[,c(1,3,4)])


mean(!is.na(results_v2$b4))
mean(!is.na(results_v2$sp))


cor(results_v2$b4, results_v2$sp, use="complete.obs")


ggplot(results_v2, aes(x = sp, y = b4)) + 
  geom_point() + 
  coord_equal() + 

...
pheno_model <- function(plotid, 
  ...
  for( p in unique(data$plotid)){
    ...
    if(length(d$doy) >= min_obs){
      ...
      #print(paste("base evi", base_index))
...
#################################################################################
ptm <- proc.time()
pheno_result_v4 <- pheno_model(data$plotid, data$evi, data$doy, data$year)
(proc.time() - ptm) / 60

res_nls_v4 <- data.frame(do.call(rbind, pheno_result_v4[[1]]))
res_spl_v4 <- data.frame(do.call(rbind, pheno_result_v4[[2]]))
results_v4 <- cbind(res_nls_v4, res_spl_v4[,c(1,3,4)])


su

mean(!is.na(results_v4$b4))
mean(!is.na(results_v4$sp))



cor.test(results_ndvi$b4, results_ndvi$sp, use="complete.obs")

cor.test(results_ndvi$sp, results_ndvi$elevation_sample, use="complete.obs")

cor.test(results_sub$b4, results_sub$Y_t, use="complete.obs")


results_v4$X_t <- results_v4$X/10000
results_v4$Y_t <- results_v4$Y/10000

results_sub <- subset(results_v4, results_v4$elevation_sample >= 500 & 
                        results_v4$elevation_sample <= 1000)


range(results_v4$sp, na.rm=TRUE)

ggplot(results_v4, aes(x = elevation_sample, y =sp)) + 
  geom_point() 

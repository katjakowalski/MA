...
p2 <- ggplot(data=GDD_SOS)+
  geom_point(aes(x=diff_LOG_EVI_TT, y=CD_LOG_EVI))

p3 <- ggplot(data=GDD_SOS)+
  geom_point(aes(x=diff_GAM_NDVI_TT, y=CD_GAM_NDVI))
p4 <- ggplot(data=GDD_SOS)+
  geom_point(aes(x=diff_LOG_NDVI_TT, y=CD_LOG_NDVI))
...
mean(GDD_SOS$CD_GAM_NDVI)
mean(GDD_SOS$CD_LOG_NDVI)
...
mean(GDD_PEP$GDD_PEP)

ggplot(GDD_SOS)+
  geom_histogram(aes(x=CD_GAM_EVI))
